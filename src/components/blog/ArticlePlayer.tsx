      ...
      <div className="flex-1 overflow-hidden relative h-16 flex items-center">
        <AnimatePresence mode="wait">
          {!isPlaying ? (
            ...
            >
              <span className="text-gray-900 font-bold mb-0.5">{messages.articlePlayer.listenToArticle}</span>
              <span className="text-xs uppercase tracking-wider text-gray-400">
                {messages.articlePlayer.audioVersion} • {messages.articlePlayer.duration}
              </span>
            </motion.div>
          ) : (
            <motion.div
